...
save_interval = 8
frame_count = 0
process_every_n_frames = 3   # jalankan face_recognition setiap 3 frame
...
while True:
    ...
    # Jalankan face_recognition setiap N frame untuk mempercepat
    if frame_count % process_every_n_frames == 0:
        ...
        for (top, right, bottom, left), enc in zip(face_locations, face_encodings):
            matches = face_recognition.compare_faces(known_encodings, enc, tolerance=0.65)
            name = "Tidak Dikenal"
            ...
            if True in matches:
                ...
                cv2.imwrite(img_path, frame)
                send_to_telegram(f"🚨 Wajah tidak dikenal! ({timestamp})", img_path)
                write_log("Wajah tidak dikenal terdeteksi.")
                control_esp32("BUZZER:ON")
                time.sleep(2)
                control_esp32("BUZZER:OFF")

            # Skala kembali koordinat wajah
    ...
    cv2.putText(frame, datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                (10, 520), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
    cv2.putText(frame, "Tekan 'Esc' untuk keluar", (680, 25),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200,200,200), 2)
    cv2.putText(frame, "Tekan 'n' untuk tambah wajah baru", (560, 520),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200,255,200), 2)

    cv2.imshow("AI Security - Face Recognition (960x540)", frame)
